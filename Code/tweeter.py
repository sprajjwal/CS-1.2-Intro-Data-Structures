...
def str_to_html(msg):
    ...
    for item in msg:
        ...
        if item[0] =='@':
            html_str += f"<a href='https://www.twitter.com/{item}'>{item}</a> "
        elif item[0]=='#':
            html_str += f"<a href='https://twitter.com/search?q=%23{item[1:]}'>{item}</a> "
        elif item[0:4].lower() == 'http':
            html_str += f"<a href='{item}'>{item}</a> "
        else:
            html_str += item + ' '
...
if __name__== '__main__':
    test = "What is this @string here. do i get capsed? ha do #abc work?"
